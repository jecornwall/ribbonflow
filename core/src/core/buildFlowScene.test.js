...
test('core index barrel re-exports buildFlowScene + agentsView', async () => {
  ...
  assert.ok(
    ...
    'the core index barrel must re-export buildFlowScene + agentsView from buildFlowScene.js',
  )
...
test('buildFlowScene: opts.showMetrics flows through to marker labels; core index still re-exports', async () => {
  const sim = createFlowSimulation(forkFlow, { initialAgents: 0 })
  ...
  // 'core index barrel re-exports' test) rather than by crashing dynamic import.
  const { readFileSync } = await import('node:fs')
  const { fileURLToPath } = await import('node:url')
  const { dirname, join } = await import('node:path')
  const dir = dirname(fileURLToPath(import.meta.url))
  ...
  assert.ok(
    src.includes("export { buildFlowScene, agentsView } from './core/buildFlowScene.js'"),
    'the core index barrel must still re-export buildFlowScene + agentsView',
  )
  const impl = await import('./buildFlowScene.js')
  assert.equal(typeof impl.buildFlowScene, 'function')
  assert.equal(typeof impl.agentsView, 'function')
